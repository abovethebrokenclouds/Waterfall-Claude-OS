...
RTAI is a mobile-first, studio-grade redesign of the original
...
> **Where this lives.** RTAI is scaffolded under `audio-analyzer/`
...
RTAI is an **audio analysis and measurement** tool. It is not a DAW,
...
RTAI is **mobile-first**. The default layout is a single graph with a
...
RTAI uses the browser's `MediaDevices` API, so it works with any
...
- **Aspirational.** Alignment of RT60 and SPL methods with the relevant IEC / ISO
  ...
  certification — RTAI is not certified to any IEC or ISO standard.
