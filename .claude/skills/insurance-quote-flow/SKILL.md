...
## Upgrade — agentic quote layer + data-prefill-first

The deterministic funnel above stays. Layer two enhancements **on top** of it
(do not discard the wizard — offer both paths):

1. **Conversational quote (AI Maya pattern).** An agentic mode that collects the
   same minimum-viable inputs through natural language, prunes questions
   dynamically, shows an indicative price, then customizes → binds. Routed
   through the Super Agent (SONNET intake; OPUS for ambiguous risk classing) —
   never a raw provider `fetch`, hardcoded model, or manual `max_tokens`
   (`superagent-conformance`). It calls the same rating service as the wizard.
2. **Data-prefill-first (Branch ~37s-to-bind pattern).** Before asking anything
   you can *buy*, enrich from name+address/VIN (property, vehicle, prior
   coverage) and **skip those questions entirely**. Ask only what materially
   changes price/eligibility *and* can't be sourced. This is the single biggest
   lever on completion time and abandonment.

Both modes converge on the same preliminary-quote → customize → bind path, feed
the `insurance-unit-economics` attach-rate dashboard, and remain gated by
`insurance-accessibility`. Pairs with `partner-service-copilot` for in-flow help.
